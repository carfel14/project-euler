...
class Solution {
public:
    long long largestProductInASeries(int digits) {
        ...
        long long actualProduct = 1;
        int zeroCount = 0;
        for (int i = 0; i < digits; i++) {
            int digit = (series[i] - '0');
            if (digits == 0) {
                zeroCount++;
                continue;
            }
        ...
        long long maxProduct = zeroCount < 1 ? actualProduct : -1;
        for (int i = digits; i < series.size(); i++) {
            int digitToAdd = (series[i] - '0');
            int digitToRemove = (series[i - digits] - '0');
            
            if (digitToAdd == 0) {
                zeroCount++;
            } else {
                actualProduct *= digitToAdd;
            }

            if (digitToRemove == 0) {
                zeroCount--;
            } else {
                actualProduct /= digitToRemove;
            }
            if (zeroCount < 1) {
                maxProduct = max(maxProduct, actualProduct);
            }
        }
...
int main() {
    Solution driver;
    long long ans = driver.largestProductInASeries(13);
    cout << ans << '\n'; 
    std::getchar();
}